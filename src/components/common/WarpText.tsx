...
const buildTextCanvas = ({
  ...
  dpr,
  props,
}: BuildCanvasArgs): HTMLCanvasElement => {
...
const syncUniforms = (program: any, props: WarpTextProps) => {
  ...
  if (props.pointerInfluence !== undefined) uniforms.uPointerInfluence.value = props.pointerInfluence;
...
export const WarpText: React.FC<WarpTextProps> = ({
  ...
  useEffect(() => {
    ...
    // Measure initial container size immediately
    const initialRect = container.getBoundingClientRect();
    const initW = Math.max(initialRect.width || 0, container.clientWidth || 0, 320);
    const initH = Math.max(initialRect.height || 0, container.clientHeight || 0, 110);
    const dpr = Math.min(window.devicePixelRatio || 1, 2);

    // Pre-rasterize text canvas immediately so texture is ready on frame 0
    const initialTextCanvas = buildTextCanvas({
      container,
      width: initW,
      height: initH,
      dpr,
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        ...
        uResolution: { value: new Float32Array([initW * dpr, initH * dpr]) },
    ...
    // Instant synchronous first frame render
    renderOnce();

    const drawText = () => {
      if (disposed || contextLost) return false;
      const rect = container.getBoundingClientRect();
      const w = Math.max(rect.width || 0, container.clientWidth || 0, initW);
      const h = Math.max(rect.height || 0, container.clientHeight || 0, initH);
